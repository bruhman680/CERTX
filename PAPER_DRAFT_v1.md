...
This irreplaceability has a formal parallel in logic. Tarski's undefinability theorem (1936) states that truth in a formal system cannot be defined within that system — any truth predicate requires a stronger meta-system. The detection gap for Type D hallucination has the same structure: no function of the output and model alone can reliably distinguish internally-coherent-and-true from internally-coherent-and-false for the Type D class. This is not a calibration failure or a data scarcity problem — it is a consequence of the same logical structure Tarski identified. Internal measurement (any fiber computed from the output) is by definition inside the system. Type D lives precisely in the gap between inside and outside. FActScore is the meta-language: a reference that can only be defined externally, in relation to ground truth outside the model.
