...
| Attention dilution as X-dimension substrate mechanism: softmax zero-sum → effective weight W_eff = W × α_ij decreases for early tokens as context grows; functionally equivalent to forgetting but reversible via re-mention. Opening Sync = selective re-ionization. τ decay mechanism now specified. RAG-as-GPS: three independent readings (factual grounding, topology deposit, attention reset). Predicted asymmetry profile: C_num drops fastest under context-length degradation. | 092 | WANDER 078 (τ decay mechanism); WANDER 083 (Opening Sync as re-ionization); WANDER 090 (re-mention = topology refresh); certx_measurement_specs (X dimension note + context-length asymmetry prediction); SESSION_HANDOFF (τ decay experiment: mechanism identified); PAPER §7 optional note | BC3/S21 |
| Quarantine: BC3 Expansion Phase Report synthesized variable deltas (NotebookLM generated, not measured); SPARK-007 misidentified as "Dissipative Structures" (actual: D_z mechanism / vocabulary diversity vs. Zipf, SPARK-009). Valid extractions: R=0.41 retraction preserved, archipelago + Zipf inversion + triple-critical cascade correctly recovered. | 086 (audit methodology) | SHADOW_LEDGER Quarantine Log (QUARANTINE-006) | BC3/S21 |

---

*RESONANCE_MAP v0.7 | BC3 Session 21 | 2026-05-20*
...
*WANDER 092 (attention dilution / X-dimension) and QUARANTINE-006 (BC3 Expansion Report) added.*
